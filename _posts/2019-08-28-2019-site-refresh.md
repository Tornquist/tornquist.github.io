...
{% include photo.html alt="NathanWeb" img="/assets/2019/08/nathanweb.png" %}
...
and the history included websites I had built for some local community groups.

{% include photo.html alt="WordPress" img="/assets/2019/08/2011.png" %}

As I begin Android development, I switched to a project-forward design:

{% include photos.html
  height="40" id="site-history"
  img1="/assets/2019/08/2013.png"
  img2="/assets/2019/08/2014.png"
%}

And finally landed with the Jekyll site.

{% include photo.html alt="Jekyll 2015" img="/assets/2019/08/2015.png" %}

Since 2015, I've tweaked the home page, but for the most part the structure
was fairly consistent.

{% include photo.html alt="Jekyll 2019" img="/assets/2019/08/2019.png" %}

In that most recent redesign in August of 2015, I migrated all the old posts
...
it would be easy to support light and dark mode. In the end, it was much easier
...
{% include photo.html alt="Light and Dark Mode" img="/assets/2019/08/light_dark.png" %}

## Remove heavy gallery plugins

When I moved the blog to Jekyll, I started using [Photoset Grid][1] with
[Colorbox][2]. Together these tools gave me the include a list of images and
control the layout using `{ layout: "312" }`. This would give three rows of pictures
with three images in the first row, one in the second, and two in the third.

While these galleries looked nice, and I really like the features they provided,
these plugins were heavy. They would listen to the window change and readjust all of
...
The photos include tool only supports a single row and up to five images, but it
is easy enough to include a few of these objects in a row to generate a grid
...
lines of css that I had neither written nor read. This made any modifications to
the site hacks at best and it was hard to really own. I was using the styles
but they were not mine.

After updating all of the layout system and removing the photo gallery plugins,
I would have a minimum, well structured site that I could actually style.

I started styling from the raw page.

{% include photo.html alt="No CSS" img="/assets/2019/08/no_css.png" %}
...
This isn't a regression, but with the focus on speed, the size of my images is
a pretty big disconnect. I do not have any build scripts or compression being
applied, and the images that I'm loading for the posts are full-size. They're
far bigger files than I need to be loading.

I am waiting to remove the large images, and instead would rather have a build
script that scales the images down as I include new gallery links to the larger
files.
